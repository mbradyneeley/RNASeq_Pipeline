...
option_list = list(
  ...
  make_option(c("-p", "--samplePhenoFile"), type="character", default=NULL, help="phenotype file from which to grab sample IDs", metavar="character")
...
# TODO: Need to adjust post run probably for PCA outliers like they did in this script by hand. 
...
if (is.null(opt$samplePhenoFile)){
  print_help(opt_parser)
  stop("Phenotype file with sample IDs in the first column need to be provided!")
} else {
  sample_pheno_file = opt$samplePhenoFile
}
...
# it includes geneid
path_count_table <- readr::read_delim(path_count_table, comment = '#')
