...
if [[ $resume == "resume" ]]; then
    if [ -d $scratchDir ]; then
        ...
        cp ./theBifrost.nf $configFile $scratchDir
        #cp -r ./bin $scratchDir
        cd $scratchDir
        nextflow -C $configFile run -with-report -with-trace -with-timeline -with-dag dag.html theBifrost.nf -resume
    else
    ...
    if [ ! -d $scratchDir ]; then
        ...
        cp -r ./bin $scratchDir
        cd $scratchDir
        nextflow -C $configFile  run -with-report -with-trace -with-timeline -with-dag dag.html theBifrost.nf
    else
